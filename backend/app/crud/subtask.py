...
def _calculate_subtask_effort(db: Session, db_subtask: models.Subtask, update_data: Optional[Dict] = None):
    ...
    if p_start:
        ...
        if not update_data or any(k in (update_data or {}) for k in ["planned_start_date", "planned_end_date", "workload_percent", "is_auto_effort", "review_days", "work_days"]):
            if p_end:
                # 予定工数 = 作業日数 * 工数比率
                # work_days が記録されている場合はそちらを直接使用する。
                # （日付から営業日数を整数で算出すると端数レビュー日数との差分が狂うため）
                if p_work_days is not None:
                    work_days_val = float(p_work_days)
                else:
                    ...
                    # 期間全体がレビュー期間以下の場合は重複とみなし全期間を作業日数とする
    ...
    if a_start and a_end:
        if not update_data or any(k in (update_data or {}) for k in ["actual_start_date", "actual_end_date", "workload_percent", "is_auto_effort", "review_start_date"]):
            ...
            for intr in db_subtask.interruptions:
                int_start = intr.interruption_date
                int_end = intr.resumption_date - timedelta(days=1) if intr.resumption_date else a_end
                overlap_start = max(a_start, int_start)
                overlap_end = min(a_end, int_end)
                if overlap_start <= overlap_end:
                    interruption_days += date_utils.get_business_days_count(overlap_start, overlap_end, holidays)
            ...
            # 実績工数 = 作業日数 * 工数比率
            # 作業日数 = 全体の営業日数 - 中断営業日数 - レビュー営業日数
            biz_days = max(0.0, float(raw_days) - interruption_days)
...
def recalculate_subtask_effort(db: Session, subtask_id: int) -> bool:
    db_subtask = db.query(models.Subtask).filter(models.Subtask.id == subtask_id, models.Subtask.is_deleted == False).first()
    if not db_subtask or not db_subtask.is_auto_effort:
        return False
...
def create_subtask(db: Session, subtask: schemas.SubtaskCreate):
    ...
    # a. Guarantee actual_start_date <= actual_end_date
    if db_subtask.actual_start_date and db_subtask.actual_end_date and db_subtask.actual_start_date > db_subtask.actual_end_date:
        db_subtask.actual_start_date = db_subtask.actual_end_date
    
    # b. Guarantee review_start_date >= actual_start_date
    if db_subtask.review_start_date and db_subtask.actual_start_date and db_subtask.review_start_date < db_subtask.actual_start_date:
        db_subtask.review_start_date = db_subtask.actual_start_date
...
def update_subtask(db: Session, subtask_id: int, subtask: schemas.SubtaskUpdate):
    ...
    if not skip_status_auto_update and new_status_id is not None and new_status_id != db_subtask.status_id:
        ...
        # 6. If moving to Removed, ensure no auto-updates overwrite current values
        ...
        a_end = update_dict.get("actual_end_date") or db_subtask.actual_end_date
        if a_start and a_end and a_start > a_end:
            # If explicit start was provided, it wins? 
            # No, usually we want to stay within end date if end date was already there.
            if "actual_start_date" in update_dict:
                update_dict["actual_start_date"] = a_end
            else:
                # In this case db_subtask.actual_start_date was what we had.
                # If we are marking it as Done and end_date is in past, we MUST shift start_date.
                update_dict["actual_start_date"] = a_end
        
        # b. review_start_date vs actual_start_date
        r_start = update_dict.get("review_start_date") or db_subtask.review_start_date
        a_start_final = update_dict.get("actual_start_date") or db_subtask.actual_start_date
        if r_start and a_start_final and r_start < a_start_final:
            update_dict["review_start_date"] = a_start_final
