...
interface TaskRowProps {
  ...
  onAddSubtask: (id: number, isShift: boolean) => void;
...
const TaskRow = memo(({ 
  ...
  const ticketLinkClassName = hasDuplicateTicketId
    ...
    : task.sync_to_azure_devops === false
  ...
  return (
    <div className={`flex group wbs-row-task ${commonRowClasses} ${checked ? 'checked' : ''}`}>
      <div
        className={`sticky left-0 flex items-center gap-1 font-medium pl-6 text-gray-700 dark:text-slate-200 wbs-cell-task transition-colors ${commonCellClasses} bg-white dark:bg-slate-900 ${checked ? 'checked' : ''}`}
      ...
      >
        ...
        <button 
          onClick={(e) => onToggleExpand(task.id, e.shiftKey)} 
        ...
        {task.memo && (
          ...
          >
            <span className="cursor-help inline-flex items-center text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 shrink-0 mx-0.5" onClick={() => onEditDetail('task', task)}>
        ...
        <button
          onClick={(e) => onAddSubtask(task.id, e.shiftKey)}
          ...
          title="サブタスクを追加 (Shift+クリックで一括作成)"
        >
          <Plus size={14} />
        </button>
      </div>
      ...
      >
        {task.ticket_id && initialData?.ticket_url_template && (
          <a
            ...
            title={hasDuplicateTicketId ? `同じチケットIDの連携対象があります (#${task.ticket_id})` : task.sync_to_azure_devops === false ? `チケットを開く (#${task.ticket_id}) - 情報を同期しない` : `チケットを開く (#${task.ticket_id})`}
            onClick={(e) => e.stopPropagation()}
          >
