...
import { Filter, X, Search, Calendar, ChevronDown, Check, RotateCcw, Settings, ChartNoAxesGantt, Link2, Share2, Download, PenTool, LayoutDashboard, Layout, ListFilter } from 'lucide-react';
...
const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 flex flex-col shrink-0 shadow-sm z-40 transition-colors">
      {/* Primary Row */}
      <div className="px-6 py-3 flex items-center gap-3 overflow-x-auto no-scrollbar">
        ...
        <MultiSelect
          ...
          className="hover:shadow-md h-[34px] min-w-[180px]"
        />

        {/* Delayed Toggle */}
        <button
          onClick={() => setFilters(prev => ({ ...prev, onlyDelayed: !prev.onlyDelayed }))}
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-xs font-bold whitespace-nowrap h-[34px] shadow-sm shrink-0 ${filters.onlyDelayed
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-xs font-bold whitespace-nowrap h-[34px] shadow-sm shrink-0 ${isFilterExpanded
          ...
          title={isFilterExpanded ? 'フィルターを隠す' : '詳細フィルターを表示'}
        >
          <ListFilter size={14} />
          詳細フィルタ
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-gray-200 dark:border-slate-700 shadow-inner h-[38px] items-center shrink-0">
          <button
                      ...
                      <label className="flex items-center justify-between cursor-pointer group px-1 py-1 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-md transition-colors">
                        ...
                        <div className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            onChange={(e) => setDisplayOptions(prev => ({ ...prev, showMarkers: e.target.checked }))}
                            className="sr-only peer"
                          />
      ...
      {isFilterExpanded && (
        <div className="px-6 py-2.5 bg-white dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-800 flex items-center gap-4 animate-in slide-in-from-top-2 duration-200 overflow-x-auto no-scrollbar shadow-inner">
          {/* Status Filter */}
          <MultiSelect
            ...
            dropdownTitle="ステータス"
            className="hover:shadow-sm h-[32px]"
          />
          ...
          {/* Search */}
          <div className="relative flex items-center min-w-[200px] h-[32px]">
            ...
            <input
              ...
              placeholder="名称で検索..."
              className="w-full pl-9 pr-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-[11px] text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all placeholder:text-gray-400 dark:placeholder:text-gray-500 font-medium shadow-sm"
            />
          </div>
        </div>
      )}
